...
describe('PythonShell', function () {
  ...
  describe('#ctor(script, options)', function () {
    it('should spawn a Python process', function (done) {
      ...
      pyshell.command.should.eql([
        'test' + sep + 'python' + sep + 'exit-code.py',
      ]);
    ...
    it('should spawn a Python process with script arguments', function (done) {
      ...
      pyshell.command.should.eql([
        'test' + sep + 'python' + sep + 'echo_args.py',
        'hello',
        'world',
      ]);
      pyshell.end(done);
  ...
  describe('#runString(script, options)', function () {
    ...
    it('should be able to execute a string of python code using promises', async function () {
      let results = await PythonShell.runString(
  ...
  describe('#run(script, options)', function () {
    ...
    it('should try to run the script and fail appropriately - async', async function () {
      try {
        ...
        throw new Error(
          `should not get here because the script should fail` + results,
        );
      } catch (err) {
        err.should.be.an.Error;
        err.exitCode.should.be.exactly(2);
    ...
    it('should include both output and error', function (done) {
      PythonShell.run('echo_hi_then_error.py', null)
        .then((results) => {
      ...
      pyshell
        ...
        .end(function (err) {
          ...
          output.should.be.exactly(
            '{"a": "b"}' + newline + 'null' + newline + '[1, 2, 3]' + newline,
          );
          done();
        });
    });
    ...
    it('should emit stderr logs as strings when mode is "text"', function (done) {
      ...
      pyshell
        .on('stderr', function (stderr) {
          ...
          count === 1 &&
            stderr.should.be.exactly(
              'DEBUG:log1:Quick zephyrs blow, vexing daft Jim.',
            );
          count++;
        })
        .on('close', function () {
  ...
  describe('.end(callback)', function () {
    ...
    it('should emit error when the program exits because of an unhandled exception', function (done) {
      ...
      pyshell.on('pythonError', function (err) {
        err.message.should.be.equalOneOf(
  ...
  describe('.parseError(data)', function () {
    ...
    it('should extend err.stack with traceback', function (done) {
      ...
      pyshell.on('pythonError', function (err) {
        err.stack.should.containEql('----- Python Traceback -----');
        err.stack.should.containEql(
          'test' + sep + 'python' + sep + 'error.py", line 4',
        );
        err.stack.should.containEql(
          'test' + sep + 'python' + sep + 'error.py", line 6',
        );
        done();
      });
    });
